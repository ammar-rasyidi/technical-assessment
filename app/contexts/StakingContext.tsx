...
export const StakingProvider: React.FC<StakingProviderProps> = ({
 ...
 const stakeTokens = async (amount: string, option: number) => {
  try {
   ...
   const valueInWei = ethers.utils.parseUnits(amount, "ether")
 ...
 const getUserStakedInfo = async () => {
  try {
   ...
   setUserStakedAmount(ethers.utils.formatUnits(amount, 18))
  } catch (error) {
 ...
 const rewardsRemaining = async () => {
  try {
   ...
   setRewardsAvailable(ethers.utils.formatUnits(rewards, 18))
  } catch (error) {
